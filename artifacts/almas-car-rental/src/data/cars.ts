export interface CarSpecs {
  engine: string;
  transmission: string;
  interior: string;
  safety: string;
}

export interface Car {
  id: number;
  name: string;
  category: "sports" | "economy" | "family" | "luxury";
  image: string;
  seats: number;
  fuelType: string;
  speed: string;
  rating: number;
  badge?: string;
  badgeColor?: string;
  description?: string;
  features?: string[];
  colors?: string[];
  specs?: CarSpecs;
  images?: string[];
}

export const cars: Car[] = [
  // Sports
  {
    id: 1,
    name: "Ford Mustang 2021",
    category: "sports",
    image: "/mustang-1.jpg",
    seats: 4,
    fuelType: "بنزين",
    speed: "240 كم/س",
    rating: 4.8,
    badge: "الأشهر",
    badgeColor: "bg-gray-700",
    features: ["محرك 2.3L تيربو", "310 حصان", "نظام Sync 3", "عجلة تحكم رياضية"],
    specs: {
      engine: "محرك 2.3 لتر تيربو، قوة 310 حصان",
      transmission: "أوتوماتيكي بـ 10 سرعات",
      interior: "تصميم رياضي كلاسيكي، عدادات رقمية قابلة للتخصيص، نظام الترفيه Sync 3",
      safety: "نظام MyKey للمراقبة، فرامل ABS، وسائد هوائية لحماية الركبة",
    },
    images: [
      "/mustang-1.jpg",
      "/mustang-2.jpg",
      "/mustang-3.jpg",
      "/mustang-4.jpg",
      "/mustang-5.jpg",
      "/mustang-6.jpg",
      "/mustang-7.jpg",
      "/mustang-8.jpg",
      "/mustang-9.jpg",
    ],
  },
  {
    id: 2,
    name: "Chevrolet Camaro 2020",
    category: "sports",
    image: "/camaro-1.jpg",
    seats: 4,
    fuelType: "بنزين",
    speed: "255 كم/س",
    rating: 4.7,
    features: ["محرك V6 3.6L", "335 حصان", "وضع Sport", "شاشة لمس"],
    specs: {
      engine: "محرك V6 سعة 3.6 لتر، قوة 335 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات",
      interior: "شاشة لمس، وضعيات قيادة متعددة، مقاعد رياضية مريحة",
      safety: "كاميرا خلفية، نظام تنبيه المنطقة العمياء، هيكل فولاذي صلب",
    },
    images: [
      "/camaro-1.jpg",
      "/camaro-2.jpg",
      "/camaro-3.jpg",
      "/camaro-4.jpg",
      "/camaro-5.jpg",
      "/camaro-6.jpg",
      "/camaro-7.jpg",
    ],
  },
  {
    id: 3,
    name: "Dodge Challenger SXT 2022",
    category: "sports",
    image: "/challenger-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "265 كم/س",
    rating: 4.9,
    badge: "الأكثر طلباً",
    badgeColor: "bg-red-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك V6 Pentastar 3.6L", "305 حصان", "فرامل Brembo", "مقاعد جلد نابا"],
    specs: {
      engine: "محرك V6 Pentastar سعة 3.6 لتر، قوة 305 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات Torqueflite",
      interior: "مقاعد جلد نابا، شاشة Uconnect مقاس 8.4 بوصة، جنوط 20 بوصة",
      safety: "نظام مساعدة الفرامل، رصد النقطة العمياء، وسائد هوائية جانبية",
    },
    images: [
      "/challenger-1.jpg",
      "/challenger-2.jpg",
      "/challenger-3.jpg",
      "/challenger-4.jpg",
      "/challenger-5.jpg",
      "/challenger-6.jpg",
      "/challenger-7.jpg",
    ],
  },
  {
    id: 4,
    name: "Chrysler 300S 2022",
    category: "sports",
    image: "/chrysler-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "235 كم/س",
    rating: 4.6,
    features: ["تصميم كلاسيكي فاخر", "V6 3.6L 300 حصان", "شاشة 8.4 بوصة", "نظام صوت Alpine"],
    specs: {
      engine: "محرك V6 سعة 3.6 لتر، قوة 300 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات مع مقابض تبديل خلف المقود",
      interior: "نظام صوتي Alpine، تصميم خارجي بلمسات سوداء، مقاعد جلدية رياضية",
      safety: "تنبيه مغادرة المسار، مثبت سرعة ذكي، حساسات ركن شاملة",
    },
    images: [
      "/chrysler-1.jpg",
      "/chrysler-2.jpg",
      "/chrysler-3.jpg",
      "/chrysler-4.jpg",
      "/chrysler-5.jpg",
      "/chrysler-6.jpg",
      "/chrysler-7.jpg",
      "/chrysler-8.jpg",
      "/chrysler-9.jpg",
      "/chrysler-10.jpg",
    ],
  },
  {
    id: 5,
    name: "Dodge Charger GT 2023",
    category: "sports",
    image: "/charger-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "250 كم/س",
    rating: 4.9,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك V6 3.6L", "300 حصان", "4 أبواب", "دفع خلفي"],
    specs: {
      engine: "محرك V6 سعة 3.6 لتر، قوة 300 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات، دفع خلفي",
      interior: "غطاء محرك رياضي، مقاعد رياضية، نظام ترفيه Uconnect",
      safety: "نظام تنبيه التصادم، كاميرا بارك فيو، التحكم الإلكتروني بالثبات",
    },
    images: [
      "/charger-1.jpg",
      "/charger-2.jpg",
      "/charger-3.jpg",
      "/charger-4.jpg",
      "/charger-5.jpg",
      "/charger-6.jpg",
    ],
  },
  {
    id: 6,
    name: "Toyota Camry 2025",
    category: "sports",
    image: "/camry-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "210 كم/س",
    rating: 4.7,
    features: ["محرك 2.5L", "201 حصان", "شاشة معلومات", "Toyota Safety Sense"],
    specs: {
      engine: "محرك 4 سلندر سعة 2.5 لتر، قوة 201 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات",
      interior: "مصابيح LED، شاشة معلومات، مقاعد مخمل، تحكم بالمقود",
      safety: "6 وسائد هوائية، نظام منع الانزلاق، حساسات خلفية",
    },
    images: [
      "/camry-1.jpg",
      "/camry-2.jpg",
      "/camry-3.jpg",
      "/camry-4.jpg",
      "/camry-5.jpg",
      "/camry-6.jpg",
      "/camry-7.jpg",
      "/camry-8.jpg",
    ],
  },
  {
    id: 28,
    name: "Kia K5 2025",
    category: "sports",
    image: "/k5-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "215 كم/س",
    rating: 4.7,
    badge: "جديد",
    badgeColor: "bg-gray-900",
    features: ["تصميم رياضي أنيق", "شاشة بانورامية منحنية", "نظام صوت Bose", "مثبت سرعة تكيفي"],
    specs: {
      engine: "محرك 4 سلندر سعة 2.5 لتر، قوة 191 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات",
      interior: "تصميم انسيابي، شاشة لمس 8 بوصة، تكييف يدوي، LED نهاري",
      safety: "مساعد تجنب الاصطدام، تنبيه انتباه السائق، وسائد أمامية",
    },
    images: [
      "/k5-1.jpg",
      "/k5-2.jpg",
      "/k5-3.jpg",
      "/k5-4.jpg",
      "/k5-5.jpg",
      "/k5-6.jpg",
      "/k5-7.jpg",
      "/k5-8.jpg",
      "/k5-9.jpg",
      "/k5-10.jpg",
      "/k5-11.jpg",
      "/k5-12.jpg",
      "/k5-13.jpg",
    ],
  },
  {
    id: 29,
    name: "Hyundai Kona N Line 2023",
    category: "sports",
    image: "/kona-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "240 كم/س",
    rating: 4.6,
    features: ["أداء N-Line رياضي", "1.6L تيربو 190 حصان", "فرامل عالية الأداء", "مقاعد N Line"],
    specs: {
      engine: "محرك 1.6 لتر تيربو، قوة 190 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات",
      interior: "لمسات رياضية حمراء، مقاعد N Line، تصميم خارجي هجومي",
      safety: "رؤية محيطية، مراقبة النقطة العمياء، رادار رصد المشاة",
    },
    images: [
      "/kona-1.jpg",
      "/kona-2.jpg",
      "/kona-3.jpg",
      "/kona-4.jpg",
      "/kona-5.jpg",
      "/kona-6.jpg",
      "/kona-7.jpg",
      "/kona-8.jpg",
      "/kona-9.jpg",
      "/kona-10.jpg",
      "/kona-11.jpg",
    ],
  },

  // Economy
  {
    id: 7,
    name: "Kia Forte 2023",
    category: "economy",
    image: "/forte-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "190 كم/س",
    rating: 4.5,
    badge: "الأوفر",
    badgeColor: "bg-green-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["Apple CarPlay & Android Auto", "مقاعد قماشية مريحة", "تحكم مناخي تلقائي", "إضاءة LED أمامية"],
    specs: {
      engine: "محرك 1.6 لتر، قوة 123 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "مقاعد قماش، شاشة معلومات، تحكم بالصوت",
      safety: "وسائد هوائية أمامية، نظام مانع الانزلاق",
    },
    images: [
      "/forte-1.jpg",
      "/forte-2.jpg",
      "/forte-3.jpg",
      "/forte-4.jpg",
      "/forte-5.jpg",
    ],
  },
  {
    id: 8,
    name: "Hyundai Elantra 2025",
    category: "economy",
    image: "/elantra-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "195 كم/س",
    rating: 4.7,
    badge: "الأحدث",
    badgeColor: "bg-blue-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["إضاءة LED كاملة", "شاشة Apple CarPlay & Android Auto", "مفتاح ذكي وتشغيل عن بُعد", "فتحة سقف"],
    specs: {
      engine: "محرك 1.5 لتر Smartstream، قيادة سلسة واقتصادية مناسبة للمدينة والطرق السريعة",
      transmission: "ناقل CVT ذكي لسلاسة فائقة في القيادة",
      interior: "جنوط 17 إنش، تحكم أوتوماتيكي بالمناخ، فتحات خلفية، مقاعد مريحة لـ5 أشخاص، إضاءة داخلية",
      safety: "مرايا كهربائية قابلة للطي، شبكة أمامية مميزة، لمسات كروم، كاميرا خلفية",
    },
    images: [
      "/elantra-1.jpg",
      "/elantra-2.jpg",
      "/elantra-3.jpg",
      "/elantra-4.jpg",
      "/elantra-5.jpg",
      "/elantra-6.jpg",
      "/elantra-7.jpg",
      "/elantra-8.jpg",
      "/elantra-9.jpg",
      "/elantra-10.jpg",
      "/elantra-11.jpg",
      "/elantra-12.jpg",
      "/elantra-13.jpg",
      "/elantra-14.jpg",
      "/elantra-15.jpg",
      "/elantra-16.jpg",
    ],
  },
  {
    id: 9,
    name: "BYD Qin Plus 2025",
    category: "economy",
    image: "/qinplus-1.jpg",
    seats: 5,
    fuelType: "هجين",
    speed: "185 كم/س",
    rating: 4.6,
    badge: "هجين",
    badgeColor: "bg-teal-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك هجين DM-i 1.5L", "شاشة دوارة", "مقاعد جلد فاخرة", "تقنيات ذكية"],
    specs: {
      engine: "محرك هجين (DM-i) 1.5 لتر مع محرك كهربائي",
      transmission: "E-CVT",
      interior: "شاشة دوارة، مقاعد جلد فاخرة، تقنيات ذكية",
      safety: "كاميرات 360، قيادة شبه ذاتية، حساسات محيطية",
    },
    images: [
      "/qinplus-1.jpg",
      "/qinplus-2.jpg",
      "/qinplus-3.jpg",
      "/qinplus-4.jpg",
      "/qinplus-5.jpg",
      "/qinplus-6.jpg",
      "/qinplus-7.jpg",
      "/qinplus-8.jpg",
      "/qinplus-9.jpg",
    ],
  },
  {
    id: 31,
    name: "BYD Destroyer 05 DM-i",
    category: "economy",
    image: "/seal05-1.jpg",
    seats: 5,
    fuelType: "هجين",
    speed: "200 كم/س",
    rating: 4.7,
    badge: "PHEV",
    badgeColor: "bg-teal-700",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 1.5L + كهربائي 290 حصان", "مدى كهربائي 55 كم", "شاشة 12.8 بوصة دوّارة", "استهلاك 4.6 لتر/100كم"],
    specs: {
      engine: "محرك 1.5 لتر هجين + محرك كهربائي، قوة إجمالية 290 حصان، استهلاك 4.6 لتر/100 كم، مدى كهربائي 55 كم",
      transmission: "E-CVT سلس وفعال",
      interior: "شاشة 12.8 بوصة دوّارة مع ملاحة وبلوتوث وOTA، نظام صوتي 6 سماعات + كاريوكي، مقاعد جلد صناعي، تكييف أوتوماتيكي مع فتحات خلفية",
      safety: "حساسات خلفية، كاميرا، مثبت سرعة، إضاءة LED أمامية ونهارية",
    },
    images: [
      "/seal05-1.jpg",
      "/seal05-2.jpg",
      "/seal05-5.jpg",
      "/seal05-6.jpg",
      "/seal05-7.jpg",
      "/seal05-8.jpg",
      "/seal05-11.jpg",
      "/seal05-12.jpg",
      "/seal05-13.jpg",
      "/seal05-14.jpg",
      "/seal05-15.jpg",
      "/seal05-16.jpg",
      "/seal05-17.jpg",
      "/seal05-18.jpg",
      "/seal05-19.jpg",
    ],
  },
  {
    id: 10,
    name: "Hyundai Accent 2024",
    category: "economy",
    image: "/accent-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "185 كم/س",
    rating: 4.4,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 1.6 لتر Smart Plus", "Apple CarPlay & Android Auto", "كاميرا خلفية + مصابيح ضباب", "بلوتوث + USB"],
    specs: {
      engine: "محرك 1.6 لتر، فئة Smart Plus، اقتصادي في استهلاك الوقود، خالية من الحوادث مع ضمان على المحرك وناقل الحركة والشاسيه",
      transmission: "أوتوماتيكي سلس",
      interior: "شاشة LCD أمامية، Android Auto وApple CarPlay، نظام ملاحة، نوافذ ومرايا كهربائية، بلوتوث + USB",
      safety: "كاميرا خلفية، مصابيح ضباب، مثبت سرعة، حساسات خلفية",
    },
    images: [
      "/accent-1.jpg",
      "/accent-2.jpg",
      "/accent-3.jpg",
      "/accent-4.jpg",
      "/accent-5.jpg",
      "/accent-6.jpg",
      "/accent-7.jpg",
      "/accent-8.jpg",
      "/accent-9.jpg",
      "/accent-10.jpg",
    ],
  },
  {
    id: 11,
    name: "Nissan Sunny 2025",
    category: "economy",
    image: "/sunny-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "180 كم/س",
    rating: 4.3,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["اقتصادية في الوقود", "سهلة القيادة", "شاشة 7 بوصة", "مفتاح ذكي"],
    specs: {
      engine: "محرك 1.6 لتر، قوة 118 حصان",
      transmission: "Xtronic CVT",
      interior: "شاشة 7 بوصة، مفتاح ذكي، كفاءة استهلاك وقود عالية",
      safety: "تحذير تصادم أمامي، كبح طوارئ، وسائد هوائية",
    },
    images: ["/sunny-1.jpg", "/sunny-2.jpg", "/sunny-3.jpg", "/sunny-4.jpg", "/sunny-5.jpg", "/sunny-6.jpg", "/sunny-7.jpg", "/sunny-8.jpg", "/sunny-9.jpg", "/sunny-10.jpg"],
  },
  {
    id: 12,
    name: "Kia Rio 2023",
    category: "economy",
    image: "/rio-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "182 كم/س",
    rating: 4.4,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["Apple CarPlay", "كاميرا خلفية", "شاشة 8 بوصة", "إضاءة LED"],
    specs: {
      engine: "محرك 4 سلندر سعة 1.6 لتر، قوة 123 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "شاشة 8 بوصة، دعم Apple CarPlay، بلوتوث، تحكم بالمقود",
      safety: "وسائد هوائية أمامية، نظام الثبات الإلكتروني، المساعدة على صعود التلال",
    },
    images: ["/rio-1.jpg", "/rio-2.jpg", "/rio-3.jpg", "/rio-4.jpg", "/rio-5.jpg", "/rio-6.jpg", "/rio-7.jpg", "/rio-8.jpg", "/rio-9.jpg", "/rio-10.jpg"],
  },
  {
    id: 13,
    name: "Kia K3 2025",
    category: "economy",
    image: "/k3-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "190 كم/س",
    rating: 4.5,
    features: ["شاشة 10.25 بوصة", "Apple CarPlay", "كاميرا خلفية", "مثبت سرعة تكيفي"],
    specs: {
      engine: "محرك 1.6 لتر، قوة 123 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "تصميم عصري، شاشة عرض مدمجة، تكييف خلفي",
      safety: "6 وسائد هوائية قياسية، نظام المساعدة في المسار",
    },
    images: ["/k3-1.jpg", "/k3-2.jpg", "/k3-3.jpg", "/k3-4.jpg", "/k3-5.jpg", "/k3-6.jpg", "/k3-7.jpg"],
  },
  {
    id: 14,
    name: "Kia K4 2025",
    category: "economy",
    image: "/k4-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "195 كم/س",
    rating: 4.6,
    badge: "جديد",
    badgeColor: "bg-gray-900",
    features: ["تصميم مستقبلي", "شاشات بانورامية", "مساحة خلفية واسعة", "نظام أمان متكامل"],
    specs: {
      engine: "محرك 2.0 لتر 4 سلندر، قوة 147 حصان",
      transmission: "IVT أوتوماتيكي",
      interior: "تصميم مستقبلي، شاشات بانورامية، مساحة خلفية واسعة",
      safety: "حزمة أمان كيا المتطورة (ADAS)، رصد النقطة العمياء",
    },
    images: ["/k4-1.jpg", "/k4-2.jpg", "/k4-3.jpg", "/k4-4.jpg", "/k4-5.jpg", "/k4-6.jpg", "/k4-7.jpg", "/k4-8.jpg", "/k4-9.jpg", "/k4-10.jpg", "/k4-11.jpg", "/k4-12.jpg", "/k4-13.jpg", "/k4-14.jpg"],
  },
  {
    id: 30,
    name: "MG5 Sedan 2024",
    category: "economy",
    image: "/mg5-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "185 كم/س",
    rating: 4.5,
    badge: "جديد",
    badgeColor: "bg-gray-800",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["تصميم عصري أنيق", "شاشة لمس 10.1 بوصة", "Apple CarPlay & Android Auto", "نظام أمان متكامل"],
    specs: {
      engine: "محرك 1.5 لتر، قوة 118 حصان",
      transmission: "CVT",
      interior: "مقاعد جلدية، فتحة سقف، شاشة لمس كبيرة",
      safety: "4 كاميرات، فرامل يد إلكترونية، وسائد هوائية",
    },
    images: ["/mg5-1.jpg", "/mg5-2.jpg", "/mg5-3.jpg", "/mg5-4.jpg", "/mg5-5.jpg"],
  },

  // Family
  {
    id: 15,
    name: "Jeep Grand Cherokee 2020",
    category: "family",
    image: "/cherokee20-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "200 كم/س",
    rating: 4.6,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 6 سلندر أوتوماتيكي", "مقاعد جلد + تحكم كهربائي", "مفتاح ذكي + زر تشغيل", "أوضاع قيادة اقتصادي/رياضي"],
    specs: {
      engine: "محرك 6 سلندر بنزين + ناقل حركة أوتوماتيكي، SUV بـ5 مقاعد مناسبة للاستخدام اليومي والسفر",
      transmission: "أوتوماتيكي مع مبدلات سرعة يدوية، أوضاع قيادة (اقتصادي / رياضي)، نظام التحكم في الجر",
      interior: "مقاعد جلد مع تحكم كهربائي، تحكم بالمناخ، مفتاح ذكي + زر تشغيل، مرايا ونوافذ وأقفال كهربائية، جنوط ألمنيوم + قضبان سقف + جناح خلفي",
      safety: "كاميرا خلفية، حساسات ركن، بلوتوث، مثبت سرعة",
    },
    images: [
      "/cherokee20-1.jpg",
      "/cherokee20-2.jpg",
      "/cherokee20-3.jpg",
      "/cherokee20-4.jpg",
      "/cherokee20-5.jpg",
      "/cherokee20-6.jpg",
      "/cherokee20-7.jpg",
      "/cherokee20-8.jpg",
      "/cherokee20-9.jpg",
      "/cherokee20-10.jpg",
    ],
  },
  {
    id: 16,
    name: "Hyundai Tucson 2025",
    category: "family",
    image: "/tucson25-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "190 كم/س",
    rating: 4.8,
    badge: "الأحدث",
    badgeColor: "bg-blue-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["تصميم عصري 2025", "شاشة بانورامية", "نظام أمان متقدم", "كاميرا 360°"],
    specs: {
      engine: "محرك 2.0 لتر 4 سلندر، قوة 156 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "فتحة سقف بانورامية، شاشة، كاميرا، تشغيل عن بعد",
      safety: "فرامل ABS، نظام المساعدة في المرتفعات، حساسات إطارات",
    },
    images: [
      "/tucson25-1.jpg",
      "/tucson25-2.jpg",
      "/tucson25-3.jpg",
      "/tucson25-4.jpg",
      "/tucson25-5.jpg",
      "/tucson25-6.jpg",
      "/tucson25-7.jpg",
      "/tucson25-8.jpg",
      "/tucson25-9.jpg",
      "/tucson25-10.jpg",
      "/tucson25-11.jpg",
    ],
  },
  {
    id: 17,
    name: "Kia Sportage 2026",
    category: "family",
    image: "/sportage-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "185 كم/س",
    rating: 4.7,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["شاشة مزدوجة منحنية", "Apple CarPlay", "فتحة سقف بانورامية", "مقاعد مريحة"],
    specs: {
      engine: "محرك 2.0 لتر، قوة 156 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "شاشة مزدوجة منحنية، Apple CarPlay، فتحة سقف بانورامية",
      safety: "نظام الأمان الشامل، وسائد هوائية متعددة، كاميرا خلفية",
    },
    images: [
      "/sportage-1.jpg",
      "/sportage-2.jpg",
      "/sportage-3.jpg",
      "/sportage-4.jpg",
      "/sportage-5.jpg",
      "/sportage-6.jpg",
      "/sportage-7.jpg",
      "/sportage-8.jpg",
    ],
  },
  {
    id: 18,
    name: "Kia Sorento 2024",
    category: "family",
    image: "/sorento-1.jpg",
    seats: 7,
    fuelType: "بنزين",
    speed: "195 كم/س",
    rating: 4.8,
    badge: "7 مقاعد",
    badgeColor: "bg-gray-800",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["7 مقاعد جلد", "شاشتين 12.3 بوصة", "تبريد مقاعد", "إضاءة محيطية"],
    specs: {
      engine: "محرك V6 سعة 3.5 لتر، قوة 272 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات، دفع رباعي",
      interior: "3 صفوف مقاعد جلد، شاشتين 12.3 بوصة، تبريد مقاعد، إضاءة محيطية",
      safety: "قيادة مساعدة ذكية، كاميرات 360، رادار النقاط العمياء الشامل",
    },
    images: [
      "/sorento-1.jpg",
      "/sorento-2.jpg",
      "/sorento-3.jpg",
      "/sorento-4.jpg",
      "/sorento-5.jpg",
      "/sorento-6.jpg",
      "/sorento-7.jpg",
      "/sorento-8.jpg",
      "/sorento-9.jpg",
      "/sorento-10.jpg",
      "/sorento-11.jpg",
      "/sorento-12.jpg",
      "/sorento-13.jpg",
      "/sorento-14.jpg",
      "/sorento-15.jpg",
      "/sorento-16.jpg",
      "/sorento-17.jpg",
      "/sorento-18.jpg",
      "/sorento-19.jpg",
    ],
  },
  {
    id: 25,
    name: "Hyundai Tucson 2024",
    category: "family",
    image: "/tucson25-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "190 كم/س",
    rating: 4.6,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["تصميم رياضي أنيق", "شاشة لمس 10.25 بوصة", "مثبت سرعة تكيفي", "بلوتوث"],
    specs: {
      engine: "محرك 2.0 لتر، قوة 156 حصان",
      transmission: "أوتوماتيكي بـ 6 سرعات",
      interior: "تصميم خارجي جريء، مثبت سرعة، شاشة ترفيه",
      safety: "وسائد هوائية أمامية، نظام منع الانغلاق، كاميرا خلفية",
    },
    images: [
      "/tucson25-1.jpg",
      "/tucson25-2.jpg",
      "/tucson25-3.jpg",
      "/tucson25-4.jpg",
      "/tucson25-5.jpg",
      "/tucson25-6.jpg",
      "/tucson25-7.jpg",
      "/tucson25-8.jpg",
      "/tucson25-9.jpg",
      "/tucson25-10.jpg",
      "/tucson25-11.jpg",
    ],
  },
  {
    id: 26,
    name: "Toyota Corolla Cross 2025",
    category: "family",
    image: "/cross-1.jpg",
    seats: 5,
    fuelType: "هجين",
    speed: "180 كم/س",
    rating: 4.7,
    badge: "هجين",
    badgeColor: "bg-teal-600",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["هايبرد 2.0L الجيل الخامس", "152 حصان", "E-CVT إلكتروني", "توفير ممتاز للوقود"],
    specs: {
      engine: "محرك 2.0 لتر هايبرد (Hybrid) الجيل الخامس، قوة 152-154 حصان، عزم 205 نيوتن متر",
      transmission: "E-CVT إلكتروني متغير باستمرار لسلاسة فائقة وهدوء أثناء القيادة",
      interior: "SUV مدمجة بـ5 مقاعد، مثالية للمدينة والرحلات الطويلة بكفاءة عالية في الوقود",
      safety: "أنظمة أمان تويوتا الكاملة لموديلات 2025 مع أحدث تقنيات الترفيه والاتصال",
    },
    images: [
      "/cross-1.jpg",
      "/cross-2.jpg",
      "/cross-3.jpg",
      "/cross-4.jpg",
      "/cross-5.jpg",
      "/cross-6.jpg",
      "/cross-7.jpg",
      "/cross-8.jpg",
      "/cross-9.jpg",
      "/cross-10.jpg",
      "/cross-11.jpg",
      "/cross-12.jpg",
      "/cross-13.jpg",
      "/cross-14.jpg",
      "/cross-15.jpg",
      "/cross-16.jpg",
      "/cross-17.jpg",
      "/cross-18.jpg",
    ],
  },
  {
    id: 27,
    name: "Kia Seltos 2024",
    category: "family",
    image: "/seltos-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "185 كم/س",
    rating: 4.5,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["شاشة 10.25 بوصة + عدادات رقمية 10.25", "سقف بانورامي", "مقاعد جلدية فاخرة", "مثبت سرعة متكيف"],
    specs: {
      engine: "محرك 1.5 لتر 4 أسطوانات بنزين، دفع أمامي (FWD)، فرامل ركن كهربائية (EPB) مع Auto Hold",
      transmission: "أوتوماتيكي سلس مع محددات وضع القيادة (Drive Modes)",
      interior: "مقاعد جلدية، سقف بانورامي، تكييف أوتوماتيكي مع فتحات خلفية، دخول ذكي بدون مفتاح، إضاءة LED متكاملة",
      safety: "مثبت سرعة متكيف، مساعد البقاء في المسار، تنبيه السلامة الأمامي، كاميرا خلفية مع حساسات أمامية وخلفية",
    },
    images: [
      "/seltos-1.jpg",
      "/seltos-2.jpg",
      "/seltos-3.jpg",
      "/seltos-4.jpg",
      "/seltos-5.jpg",
      "/seltos-6.jpg",
      "/seltos-7.jpg",
      "/seltos-8.jpg",
      "/seltos-9.jpg",
      "/seltos-10.jpg",
      "/seltos-11.jpg",
      "/seltos-12.jpg",
      "/seltos-13.jpg",
      "/seltos-14.jpg",
      "/seltos-15.jpg",
      "/seltos-16.jpg",
      "/seltos-17.jpg",
      "/seltos-18.jpg",
      "/seltos-19.jpg",
      "/seltos-20.jpg",
    ],
  },

  // Luxury
  {
    id: 19,
    name: "GMC Yukon Denali 2024",
    category: "luxury",
    image: "/yukon-1.jpg",
    seats: 7,
    fuelType: "بنزين",
    speed: "220 كم/س",
    rating: 4.9,
    badge: "الأفضل",
    badgeColor: "bg-gray-900",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["7 مقاعد فاخرة", "شاشة 15 بوصة", "نظام صوت Bose", "Super Cruise"],
    specs: {
      engine: "محرك V8 سعة 6.2 لتر، قوة 420 حصان، عزم 623 نيوتن متر",
      transmission: "أوتوماتيكي بـ 10 سرعات، نظام دفع رباعي (4WD)",
      interior: "مقاعد جلدية فاخرة، شاشة عرض كبيرة، نظام صوتي Bose، تبريد وتدفئة المقاعد",
      safety: "نظام القيادة الذاتية Super Cruise، فرملة طوارئ تلقائية، كاميرات رؤية محيطية 360 درجة",
    },
    images: ["/yukon-1.jpg", "/yukon-2.jpg", "/yukon-3.jpg", "/yukon-4.jpg", "/yukon-5.jpg", "/yukon-6.jpg", "/yukon-7.jpg", "/yukon-8.jpg", "/yukon-9.jpg", "/yukon-10.jpg", "/yukon-11.jpg", "/yukon-12.jpg", "/yukon-13.jpg", "/yukon-14.jpg", "/yukon-15.jpg", "/yukon-16.jpg", "/yukon-17.jpg", "/yukon-18.jpg", "/yukon-19.jpg", "/yukon-20.jpg", "/yukon-21.jpg", "/yukon-22.jpg", "/yukon-23.jpg", "/yukon-24.jpg"],
  },
  {
    id: 20,
    name: "Land Cruiser GXR 2025",
    category: "luxury",
    image: "/lc-1.jpg",
    seats: 7,
    fuelType: "بنزين",
    speed: "210 كم/س",
    rating: 5.0,
    badge: "الأقوى",
    badgeColor: "bg-gray-800",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["V6 توين تيربو 409 حصان", "أنظمة طرق وعرة", "ثلاجة داخلية", "إضاءة عتبات ليمتد"],
    specs: {
      engine: "محرك V6 سعة 3.5 لتر توين تيربو، قوة 409 حصان",
      transmission: "أوتوماتيكي بـ 10 سرعات، دفع رباعي مع أنظمة طرق وعرة",
      interior: "شاشة ملاحة، فتحة سقف، ثلاجة داخلية، إضاءة عتبات ليمتد",
      safety: "نظام تويوتا للأمان (TSS)، وسائد هوائية كاملة، نظام الزحف والدوران",
    },
    images: ["/lc-1.jpg", "/lc-2.jpg", "/lc-3.jpg", "/lc-4.jpg", "/lc-5.jpg", "/lc-6.jpg", "/lc-7.jpg", "/lc-8.jpg", "/lc-9.jpg", "/lc-10.jpg"],
  },
  {
    id: 21,
    name: "Chevrolet Tahoe Premier 2024",
    category: "luxury",
    image: "/tahoe-1.jpg",
    seats: 8,
    fuelType: "بنزين",
    speed: "205 كم/س",
    rating: 4.8,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["8 مقاعد فاخرة", "شاشات ترفيهية خلفية", "V8 5.3L 355 حصان", "تعليق مغناطيسي"],
    specs: {
      engine: "محرك V8 سعة 5.3 لتر، قوة 355 حصان",
      transmission: "أوتوماتيكي بـ 10 سرعات، دفع رباعي",
      interior: "شاشات خلفية، مقاعد منفصلة في الصف الثاني، نظام تعليق مغناطيسي مريح",
      safety: "تنبيه تصادم أمامي، مساعد البقاء في المسار، رادار كشف المشاة",
    },
    images: ["/tahoe-1.jpg", "/tahoe-2.jpg", "/tahoe-3.jpg", "/tahoe-4.jpg", "/tahoe-5.jpg", "/tahoe-6.jpg", "/tahoe-7.jpg", "/tahoe-8.jpg", "/tahoe-9.jpg", "/tahoe-10.jpg", "/tahoe-11.jpg", "/tahoe-12.jpg", "/tahoe-13.jpg"],
  },
  {
    id: 22,
    name: "Range Rover SVR 2022",
    category: "luxury",
    image: "/rrsvr-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "280 كم/س",
    rating: 4.9,
    badge: "محرك رياضي",
    badgeColor: "bg-red-700",
    features: ["V8 سوبرتشارجر 575 حصان", "فتحة سقف بانورامية", "ديكورات كربون", "فرامل Brembo"],
    specs: {
      engine: "محرك V8 سوبر تشارجد سعة 5.0 لتر، قوة 575 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات مع وضعيات قيادة رياضية",
      interior: "مقاعد رياضية SVR، ديكورات ألياف الكربون، سقف بانورامي",
      safety: "فرامل بريمبو عالية الأداء، نظام الثبات على المنعطفات، حساسات محيطية",
    },
    images: ["/rrsvr-1.jpg", "/rrsvr-2.jpg", "/rrsvr-3.jpg", "/rrsvr-4.jpg", "/rrsvr-5.jpg", "/rrsvr-6.jpg", "/rrsvr-7.jpg", "/rrsvr-8.jpg", "/rrsvr-9.jpg", "/rrsvr-10.jpg", "/rrsvr-11.jpg", "/rrsvr-12.jpg", "/rrsvr-13.jpg"],
  },
  {
    id: 23,
    name: "BMW 228i 2022",
    category: "luxury",
    image: "/bmw228-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "250 كم/س",
    rating: 4.8,
    features: ["شاشة BMW iDrive 8", "دفع كلي xDrive", "إضاءة داخلية محيطية", "مقاعد SensaTec"],
    specs: {
      engine: "محرك 4 سلندر تيربو سعة 2.0 لتر، قوة 228 حصان",
      transmission: "أوتوماتيكي رياضي بـ 8 سرعات، دفع كلي xDrive",
      interior: "شاشة عدادات رقمية، نظام iDrive المتطور، إضاءة داخلية محيطية، مقاعد رياضية SensaTec",
      safety: "نظام المساعدة في القيادة النشط، كشف النقاط العمياء، تنبيه مغادرة المسار",
    },
    images: ["/bmw228-1.jpg", "/bmw228-2.jpg", "/bmw228-3.jpg", "/bmw228-4.jpg", "/bmw228-5.jpg", "/bmw228-6.jpg", "/bmw228-7.jpg", "/bmw228-8.jpg", "/bmw228-9.jpg", "/bmw228-10.jpg", "/bmw228-11.jpg", "/bmw228-12.jpg", "/bmw228-13.jpg", "/bmw228-14.jpg", "/bmw228-15.jpg", "/bmw228-16.jpg"],
  },
  {
    id: 24,
    name: "Mercedes E Class 2022",
    category: "luxury",
    image: "/merc-e-1.jpg",
    seats: 5,
    fuelType: "هجين",
    speed: "250 كم/س",
    rating: 4.9,
    badge: "الأكثر طلباً",
    badgeColor: "bg-gray-700",
    features: ["شاشات عرض مزدوجة", "نظام MBUX", "إضاءة 64 لون", "جلود فاخرة"],
    specs: {
      engine: "محرك تيربو سعة 2.0 لتر، قوة 197 حصان",
      transmission: "أوتوماتيكي بـ 9 سرعات (9G-TRONIC)",
      interior: "شاشات عرض مزدوجة، نظام MBUX، إضاءة 64 لون، جلود فاخرة",
      safety: "نظام Pre-Safe، مكابح طوارئ نشطة، رادار أمامي ذكي",
    },
    images: ["/merc-e-1.jpg", "/merc-e-2.jpg", "/merc-e-3.jpg", "/merc-e-4.jpg", "/merc-e-5.jpg", "/merc-e-6.jpg", "/merc-e-7.jpg", "/merc-e-8.jpg", "/merc-e-9.jpg", "/merc-e-10.jpg", "/merc-e-11.jpg", "/merc-e-12.jpg", "/merc-e-13.jpg", "/merc-e-14.jpg", "/merc-e-15.jpg", "/merc-e-16.jpg", "/merc-e-17.jpg", "/merc-e-18.jpg", "/merc-e-19.jpg", "/merc-e-20.jpg"],
  },

  // New Economy Cars
  {
    id: 32,
    name: "Nissan Sentra 2023",
    category: "economy",
    image: "/sentra-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "185 كم/س",
    rating: 4.4,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 1.6L 120 حصان", "Apple CarPlay", "كاميرا خلفية", "شاشة لمس"],
    specs: {
      engine: "محرك 4 سلندر سعة 1.6 لتر، قوة 120 حصان",
      transmission: "CVT أوتوماتيكي",
      interior: "شاشة لمس، Apple CarPlay، تكييف تلقائي، مقاعد قماشية مريحة",
      safety: "كاميرا خلفية، وسائد هوائية أمامية، نظام ABS، مثبت السرعة",
    },
    images: ["/sentra-1.jpg", "/sentra-2.jpg", "/sentra-3.jpg", "/sentra-4.jpg", "/sentra-5.jpg", "/sentra-6.jpg", "/sentra-7.jpg", "/sentra-8.jpg", "/sentra-9.jpg"],
  },
  {
    id: 33,
    name: "Toyota Corolla 2025",
    category: "economy",
    image: "/car-corolla2025.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "195 كم/س",
    rating: 4.6,
    badge: "الأكثر مبيعاً",
    badgeColor: "bg-blue-700",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 1.8L", "شاشة لمس 8 بوصة", "Apple CarPlay", "Toyota Safety Sense"],
    specs: {
      engine: "محرك 4 سلندر سعة 1.8 لتر، قوة 139 حصان",
      transmission: "أوتوماتيكي CVT",
      interior: "شاشة لمس 8 بوصة، Apple CarPlay & Android Auto، تحكم مناخي تلقائي",
      safety: "Toyota Safety Sense، 8 وسائد هوائية، نظام الثبات الإلكتروني، كاميرا خلفية",
    },
    images: ["/car-corolla2025.jpg", "/car-corolla2025-2.jpg", "/car-corolla2025-3.jpg", "/car-corolla2025-int-1.jpg", "/car-corolla2025-int-2.jpg", "/car-corolla2025-int-3.jpg"],
  },
  {
    id: 37,
    name: "Hyundai Accent 2022",
    category: "economy",
    image: "/accent22-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "180 كم/س",
    rating: 4.3,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 1.6L", "120 حصان", "Apple CarPlay & Android Auto", "فتحة سقف"],
    specs: {
      engine: "محرك 4 سلندر إنلاين سعة 1.6 لتر، قوة 120 حصان، عزم 113 نيوتن.م، دفع أمامي",
      transmission: "أوتوماتيكي CVT أو يدوي 6 سرعات",
      interior: "شاشة 5 بوصة، بلوتوث، مثبت سرعة، تكييف، زجاج كهربائي، Apple CarPlay & Android Auto، فتحة سقف، تشغيل بمفتاح",
      safety: "نظام الثبات الإلكتروني (ESC)، نظام التحكم بالجر، وسائد هوائية",
    },
    images: ["/accent22-1.jpg", "/accent22-2.jpg", "/accent22-3.jpg", "/accent22-4.jpg", "/accent22-5.jpg", "/accent22-6.jpg", "/accent22-7.jpg", "/accent22-8.jpg", "/accent22-9.jpg", "/accent22-10.jpg"],
  },

  // New Family Car
  {
    id: 34,
    name: "Jeep Grand Cherokee 2024",
    category: "family",
    image: "/cherokee24-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "210 كم/س",
    rating: 4.8,
    badge: "جديد",
    badgeColor: "bg-gray-900",
    colors: ["#ffffff", "#1a1a1a"],
    features: ["V6 3.6L 293 حصان", "دفع رباعي 4x4", "شاشة 10.1 بوصة", "كاميرا 360°"],
    specs: {
      engine: "محرك V6 سعة 3.6 لتر، قوة 293 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات، دفع رباعي متكامل",
      interior: "شاشة Uconnect 10.1 بوصة، مقاعد جلدية، تسخين مقاعد، فتحة سقف بانورامية",
      safety: "كاميرا 360°، رادار كشف الأجسام، وسائد هوائية متعددة، مساعد البقاء في المسار",
    },
    images: [
      "/cherokee24-1.jpg",
      "/cherokee24-2.jpg",
      "/cherokee24-3.jpg",
      "/cherokee24-4.jpg",
      "/cherokee24-5.jpg",
      "/cherokee24-6.jpg",
      "/cherokee24-7.jpg",
      "/cherokee24-8.jpg",
    ],
  },

  // New Sports Car
  {
    id: 35,
    name: "Toyota Camry 2024",
    category: "sports",
    image: "/camry24-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "210 كم/س",
    rating: 4.7,
    colors: ["#ffffff", "#1a1a1a"],
    features: ["محرك 2.5L", "تصميم رياضي 2024", "شاشة 9 بوصة", "Toyota Safety Sense"],
    specs: {
      engine: "محرك 4 سلندر سعة 2.5 لتر، قوة 203 حصان",
      transmission: "أوتوماتيكي بـ 8 سرعات",
      interior: "شاشة 9 بوصة، Apple CarPlay، مقاعد مخمل فاخرة، تحكم مناخي مزدوج",
      safety: "Toyota Safety Sense 2.0، 8 وسائد هوائية، نظام منع الانزلاق، كاميرا خلفية HD",
    },
    images: [
      "/camry24-1.jpg",
      "/camry24-2.jpg",
      "/camry24-3.jpg",
      "/camry24-4.jpg",
      "/camry24-5.jpg",
      "/camry24-6.jpg",
      "/camry24-7.jpg",
      "/camry24-8.jpg",
      "/camry24-9.jpg",
      "/camry24-10.jpg",
      "/camry24-11.jpg",
    ],
  },

  // New Luxury Car
  {
    id: 36,
    name: "BMW 525i xDrive",
    category: "luxury",
    image: "/bmw525-1.jpg",
    seats: 5,
    fuelType: "بنزين",
    speed: "250 كم/س",
    rating: 4.9,
    badge: "جديد",
    badgeColor: "bg-gray-900",
    features: ["محرك 2.0L تيربو 252 حصان", "دفع كلي xDrive", "شاشة iDrive 12.3 بوصة", "مقاعد جلد ناباصوفت"],
    specs: {
      engine: "محرك تيربو سعة 2.0 لتر، قوة 252 حصان",
      transmission: "أوتوماتيكي رياضي Steptronic بـ 8 سرعات، دفع كلي xDrive",
      interior: "شاشة iDrive 12.3 بوصة، مقاعد جلد ناباصوفت، نظام صوت Harman Kardon، إضاءة محيطية",
      safety: "نظام مساعدة القيادة BMW، كشف النقاط العمياء، تنبيه مغادرة المسار، رادار أمامي",
    },
    images: ["/bmw525-1.jpg", "/bmw525-2.jpg", "/bmw525-3.jpg", "/bmw525-4.jpg", "/bmw525-5.jpg", "/bmw525-6.jpg", "/bmw525-7.jpeg", "/bmw525-8.jpg", "/bmw525-9.jpg", "/bmw525-10.jpg", "/bmw525-11.jpg", "/bmw525-12.jpg", "/bmw525-13.jpg", "/bmw525-14.jpg"],
  },
];

export const getCategoryLabel = (category: Car["category"]) => {
  const labels = {
    sports: "الرياضية",
    economy: "السيارات الاقتصادية",
    family: "العائلية",
    luxury: "الفاخرة",
  };
  return labels[category];
};
